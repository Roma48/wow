...
if ( ! function_exists( 'onepress_posted_on' ) ) {
    ...
    function onepress_posted_on()
    {
        ...
        $posted_on = strftime("%d %B, %Y", strtotime(get_the_date()));
        ...
        if ('post' === get_post_type()) {
            ...
            if ($categories_list && onepress_categorized_blog()) {
                $posted_in = '<span class="cat-links">' . $categories_list . '</span>'; // WPCS: XSS OK.
            }
        }

        echo '<span class="posted-on">' . $posted_on . '</span> | <span>' . $posted_in . '</span>'; // WPCS: XSS OK.
